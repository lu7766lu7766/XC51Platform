 ...
 */
class Super_Fto implements IProHandle {
	private static _mInstance: Super_Fto;
	public static get getInstance(): Super_Fto {
	 ...
	 * mold:金额类型(多个用逗号隔开,1=>充值金额 2=>中奖金额  3=>优惠金额）
	 */
	public sendHttp(list:Array<Strand>,dryingData:DryingData,t:number, select:number[],czType:string,x:number): void {
		let url: string = HTTPRequest.getInstance.httpHeadUrl + "/super_Fto.php";
		let a = new Array();
		let zs:number = 0;
		let m:number = 0;
		let obj:Strand;
		...
		let content = `id=${UserData.getInstance.userId}&s=${jsonstr}&r=${dryingData.yltc}&d=${dryingData.faxy}&type=${dryingData.type}&z=${zs}&m=${m}&t=${t}&mold=${czType}&rkey=${GameValue.orderKey}&x=${x}&v=${GameValue.verPhp}`;
