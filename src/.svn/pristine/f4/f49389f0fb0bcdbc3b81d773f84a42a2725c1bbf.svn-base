...
返回t和数据列表
 */
class SPBK_One implements IProHandle {
	private static _mInstance: SPBK_One;
	public static get getInstance(): SPBK_One {
	...
	private constructor() {
		...
		GTimerMag.getInstance.addTimerTask("SPBK_One", 9999999, GameValue.httpSendTime, () => {
	...
	public sendHttp(type:number,obj:egret.DisplayObjectContainer,bool?:boolean): void {
		...
		let url: string = HTTPRequest.getInstance.httpHeadUrl + "/super_Bk_One.php";
		let content = `t=${type}&v=${GameValue.verPhp}`;
