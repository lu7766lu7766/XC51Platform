/**篮球选择日期 */
class selectDayBasketBall extends egret.DisplayObjectContainer {
	private static _mInstance: selectDayBasketBall;
	public static get getInstance(): selectDayBasketBall {
	...
	/**选择列表 */
	private _mListBGBGBG: GHashMap<selectdayItem1>;
	public getlist(): GHashMap<selectdayItem1> {
	...
	public constructor() {
		...
		this._mListBGBGBG = new GHashMap<selectdayItem1>();
	...
	public initAllRedBg(): void {
		let len: number =selectDayBasketBallData.selectDay.length;
		let obj: selectdayItem1;
		let ypoin: number = 0;
		for (let i = 1; i <= len; i++) {
			...
			obj.setid(i);
			let timer: string = selectDayBasketBallData.selectDay[i - 1] + "";
...
class selectdayItem1 extends egret.DisplayObjectContainer {
	public constructor() {
		super();
	...
	private onTouch(): void {
		...
		let timer: number = selectDayBasketBallData.selectDay[this.myid - 1];
