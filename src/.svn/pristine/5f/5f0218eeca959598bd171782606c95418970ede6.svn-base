 ...
 */
class Super_Bk_Order implements IProHandle {
	private static _mInstance: Super_Bk_Order;
	public static get getInstance(): Super_Bk_Order {
	...
	public sendHttp(list:Array<Strand>,dryingData:DryingData,t:number, select:number[],czType:string): void {
		let url: string = HTTPRequest.getInstance.httpHeadUrl + "/super_Bk_Order.php";
		let a = new Array();
		let zs:number = 0;
		let m:number = 0;
		let obj:Strand;
