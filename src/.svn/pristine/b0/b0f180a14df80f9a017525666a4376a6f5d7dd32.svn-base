 ...
 */
class Super_Ftc implements IProHandle {
	private static _mInstance: Super_Ftc;
	public static get getInstance(): Super_Ftc {
	 ...
	 * mold:金额类型(多个用逗号隔开,1=>充值金额 2=>中奖金额  3=>优惠金额）
	 */
	public sendHttp(dataItem:Array<zqObjData>,dryingData:DryingData,t:number, select:number[],b:number,m:number,czType:string,x:number): void {
		let url: string = HTTPRequest.getInstance.httpHeadUrl + "/super_Ftc.php";
		let strChuan:string = "";
		...
		let content = `id=${UserData.getInstance.userId}&s=${jsonstr}&r=${dryingData.yltc}&d=${dryingData.faxy}&type=${dryingData.type}&b=${b}&m=${m * 100}&t=${t}&mold=${czType}&l=${strChuan}&rkey=${GameValue.orderKey}&x=${x}&v=${GameValue.verPhp}`;
