 ...
 */
class FT_Order_One implements IProHandle {
	private static _mInstance: FT_Order_One;
	public static get getInstance(): FT_Order_One {
	...
	public sendHttp(dataItem: Array<zqObjData>, dryingData: DryingData, t: number, select: number[], b: number, m: number, czType: string): void {
		let url: string = HTTPRequest.getInstance.httpHeadUrl + "/ft_Order_One.php";
		let strChuan: string = "";
	...
	public backHTTP(res: boolean, httpObj: egret.HttpRequest, data: HttpData): void {
		if (res == true && httpObj.response != "") {//请求成功
			...
			if (text["res"] != "0") {
				...
				} else if (text["res"] == "1001") {//某一场赛事购买截止
