class ThreeBox extends egret.DisplayObjectContainer{
    private static _mInstance: ThreeBox;
	public static get getInstance(): ThreeBox {
    ...
    constructor(){
        ...
        this._titleImg.y = 84-40+GameValue.adaptationScreen;
    ...
    private touchDown(e:egret.TouchEvent):void{
        if(e.target == this._titleText || e.target == this._titleImg){//点击标题
            ...
            this.ClickClear();
        } else if(e.target == this._question) {//问题
            ListThreeExplain.getInstance.show();
        } else if(e.target == this._history) {//历史
    ...
    public updataHistory():void{
        let objHeight=0;
        let index=0;
        let item = HistoryAwardsDataMrg.getInstance.getHistory3List();
...
class ThreeHistoryInfo extends egret.DisplayObjectContainer{
    private _text:egret.TextField;

    constructor(){
        super();

        this._text = ToolMrg.getText(28,0,32,0x333333);
        this.addChild(this._text);
        this._text.height = 70;
        this._text.verticalAlign = egret.VerticalAlign.MIDDLE;

        this.setDB();
    }

    private _data:HistoryAwardsData
    public aa(data:HistoryAwardsData):void{
        data.qs
        let str = "";
...
class Circle extends egret.DisplayObjectContainer{
    ...
    constructor(id,type){
        ...
        this._num = ToolMrg.getText(0,0,36,0xF72E52,80);
        this.addChild(this._num);
        this._num.height = 80;
        this._num.verticalAlign = egret.VerticalAlign.MIDDLE;
        this._num.textAlign = egret.HorizontalAlign.CENTER;
        this._num.text = id+"";

        this.anchorOffsetX = this.width*0.5;
        this.anchorOffsetY = this.height*0.5;

        RES.getResByUrl("resource/assets/images/ui/hmbg_home@2x.png",(e)=>{},this);
        this.setBtn();
    }
    ...
    private changeItem():void{
        if(this.type<10){
            if (this.type == 0) {
                let item = ThreeBox.getInstance.oneItem;
                if (this._numType)
                    item.push(this._id);
                else {
                    let aa = [];
                ...
                let item = ThreeBox.getInstance.twoItem;
                if (this._numType)
                    item.push(this._id);
                else {
                    let aa = [];
                ...
                let item = ThreeBox.getInstance.threeItem;
                if (this._numType)
                    item.push(this._id);
                else {
                    let aa = [];
            ...
            if(this.type==10){
                let item = FiveBox.getInstance.oneItem;
                if (this._numType)
                    item.push(this._id);
                else {
                    let aa = [];
                ...
                let item = FiveBox.getInstance.twoItem;
                if (this._numType)
                    item.push(this._id);
                else {
                    let aa = [];
                ...
                let item = FiveBox.getInstance.threeItem;
                if (this._numType)
                    item.push(this._id);
                else {
                    let aa = [];
                ...
                let item = FiveBox.getInstance.fourItem;
                if (this._numType)
                    item.push(this._id);
                else {
                    let aa = [];
                ...
                let item = FiveBox.getInstance.fiveItem;
                if (this._numType)
                    item.push(this._id);
                else {
                    let aa = [];
    ...
    /**改变图片 */
    private setBtn():void{
        let str:string = this._numType?"hmbg_home@2x.png":"hmbg_nor_home@2x.png";
        RES.getResByUrl("resource/assets/images/ui/"+str,(e)=>{this._img.$setBitmapData(e);},this);
    }

    private changeSome():void{
        this._img.scaleX
        this._img.touchEnabled = false;
        egret.Tween.get(this).to({scaleX:0.8,scaleY:0.8},100).call((e)=>{
            egret.Tween.get(this).to({scaleX:1,scaleY:1},200,egret.Ease.circInOut).call(()=>{
