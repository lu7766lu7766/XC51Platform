/**更多玩法 串关 篮球 */
class B1more extends egret.DisplayObjectContainer{
    private static _mInstance: B1more;
	public static get getInstance(): B1more {
    ...
    private joinBlock():void{
        //0 - 1
        for(let i=0;i<2;i++){
            let obj = new BasketballBlock(i,0,240,100,0,19,57);
        ...
        //2 - 3
        for(let i=2;i<4;i++){
            let obj = new BasketballBlock(i,1,240,100,0,19,57);
            this._mContain.addChild(obj);
            this._item.Gput(i,obj);
        ...
        //4 - 9
        for(let i=4;i<10;i++){
            let obj = new BasketballBlock(i,1,100,100,0);
            this._mContain.addChild(obj);
            this._item.Gput(i,obj);
            obj.x = 76 + (i-4)*(100+4);
            obj.y = 256;
